...
for i in range(len(kondisi)):
    ...
    for j in range(len(brand)):
        query = c.execute("SELECT min(price) FROM handphone WHERE (brand == '"+brand[j]+"' and city =='"+city+"' and label == '"+kondisi[i]+"')")
        ftch = c.fetchall()
        if not ftch:
            hsl = 0
        else:
...
#simpan file
with open('dictionary_handphone_minlimabrand.txt', 'w') as fp:
    json.dump(data, fp)
